...
void send_level_request(int retrans,uint16 level )
{
	printf("Sending Level\r\n");
  uint16 resp;
  uint16 delay;
  struct mesh_generic_request req;
...
void handle_button_press(int button)
{
	if(button)
	{
	level  = GESTURE_THRESHOLD_INCREASE;
	 DI_Print("THRESHOLD INCREASED ", DI_ROW_STATUS);
	}
	 else
	 {
		 level = GESTURE_THRESHOLD_DECREASE;
	DI_Print("THRESHOLD DECREASED", DI_ROW_STATUS);
	 }
	send_level_request(0,level);
...
void handle_very_long_press(int button)
{
	...
	if(button)
		{
		level  = GESTURE_SENSETIVITY1_INCREASE;
		 DI_Print("SENSETIVITY INCREASED ", DI_ROW_STATUS);
		}
		 else
		 {
			 level = GESTURE_SENSETIVITY1_DECREASE;
		DI_Print("SENSETIVITY DECREASED", DI_ROW_STATUS);
		 }
 send_level_request(0,level);
...
void set_device_name(bd_addr *pAddr)
{
  ...
  sprintf(name, "CENTRAL DEVICE %x:%x", pAddr->addr[1], pAddr->addr[0]);
