...
static uint16 level = 0;  /* lightness level converted from percentage to actual value, range 0..65535*/
static bool handleGestureFlag = 0;

static uint16_t friend_address;

/* button press timestamps for long/short press detection */
static uint32 pb0_press;
static uint32 pb1_press;
struct gecko_msg_mesh_lpn_friendship_established_evt_t *friend ;
...
#define EXT_SIGNAL_APDS_PIN             0x02
...
const uint8 CENTRAL_UUID[16]=    {0x53,0x69,0x6C,0x61,0x62,0x73,0x44,0x65,0x76,0x2D,0xE1,0x7E,0xA9,0x9F,0xFD,0x90};
const uint8 WHEELCHAIR_UUID[16]= {0x53,0x69,0x6C,0x61,0x62,0x73,0x44,0x65,0x76,0x2D,0x60,0xF2,0xB5,0x57,0x0B,0x00};
...
#define LEFT					0x10
#define RIGHT					0x20
#define MOTOR_SPEED_DECREASE	0x40
#define MOTOR_SPEED_INCREASE	0x80

#define GESTURE_THRESHOLD_INCREASE  0x100
#define GESTURE_SENSETIVITY1_INCREASE 0x200
#define GESTURE_SENSETIVITY2_INCREASE  0x400
...
struct Sensor_state{
 ...
 uint16_t gest_thresold;
 uint16_t gest_sensetivity_1;
}Sensor_state;
...
else if (pin == APDS_INT_PIN)
	{
	printf("Gesture Detected\r\n");

    gecko_external_signal(EXT_SIGNAL_APDS_PIN);
	}
}
...
void enable_button_interrupts(void)
{
  ...
  GPIO_ExtIntConfig(APDS_INT_PORT, APDS_INT_PIN, APDS_INT_PIN, true, true, true);

  /* register the callback function that is invoked when interrupt occurs */
  GPIOINT_CallbackRegister(BSP_BUTTON0_PIN, gpioint);
  GPIOINT_CallbackRegister(BSP_BUTTON1_PIN, gpioint);
  GPIOINT_CallbackRegister(APDS_INT_PIN, gpioint);
}
static int gesture_state_load(void)
{
	char str[10], str1[10];
  struct gecko_msg_flash_ps_load_rsp_t* pLoad;

  pLoad = gecko_cmd_flash_ps_load(0x4004);
  printf("Loaded persistent data\n");

  if (pLoad->result) {
    memset(&Sensor_state, 0, sizeof(Sensor_state));
    Sensor_state.gest_sensetivity_1 = GESTURE_SENSITIVITY_1;

    Sensor_state.gest_thresold= GESTURE_THRESHOLD_OUT;
	  sprintf(str,"THRESOLD= %d",Sensor_state.gest_thresold);
	  DI_Print(str, DI_ROW_THRESHOLD );
	sprintf(str1,"SENSETIVITY= %d",Sensor_state.gest_sensetivity_1);
	 DI_Print(str1, DI_ROW_SENSETIVITY);
    return -1;
  }
  	  memcpy(&Sensor_state, pLoad->value.data, pLoad->value.len);
  	  printf("Thresold is %d\r\n Sensetivity 1 is %d\r\n Sensitivity2 is \r\n %d",Sensor_state.gest_thresold,Sensor_state.gest_sensetivity_1);
  	  sprintf(str,"THRESOLD= %d",Sensor_state.gest_thresold);
  	  DI_Print(str, DI_ROW_THRESHOLD );
  	sprintf(str1,"SENSETIVITY= %d",Sensor_state.gest_sensetivity_1);
  	 DI_Print(str1, DI_ROW_SENSETIVITY);
...
static int gesture_state_store(void)
{
  struct gecko_msg_flash_ps_save_rsp_t* pSave;

  pSave = gecko_cmd_flash_ps_save(0x4004, sizeof(Sensor_state), (const uint8*)&Sensor_state);
  printf("Saved Flash PS\r\n");
  if (pSave->result) {
...
void send_level_request(int retrans)
{
  uint16 resp;
  uint16 delay;
  struct mesh_generic_request req;
  ...
  resp = gecko_cmd_mesh_generic_client_set(
	MESH_GENERIC_LEVEL_CLIENT_MODEL_ID,
     ...
     delay,
     0,     // flags
	 mesh_generic_request_level, // type
     2,     // param len
     (uint8*)&req.level   /// parameters data
     )->result;
  if (resp) {
    printf("gecko_cmd_mesh_generic_client_publish failed,code %x\r\n", resp);
  } else {
...
void handle_button_press(int button)
{
 ...
 if (button == 1){
	 ...
	 level = MOTOR_SPEED_INCREASE;
	 printf("Motor Speed Increased by 1\r\n");

 }

else {
	   level= MOTOR_SPEED_DECREASE;
	   printf("Motor Speed decreased by 1\r\n");

}

 /* send the request (lightness request is sent only once in this example) */
 send_level_request(0);
...
void handleGesture()
 {
   ...
   if (isGestureAvailable())
	   gesture_state_load();
   {
	  // DI_Print("", DI_ROW_DIRECTION);
     switch (readGesture(Sensor_state.gest_sensetivity_1,Sensor_state.gest_thresold))
     {
       ...
       case DIR_UP:
     	  printf("Forward\r\n");
     	  level= FORWARD;
     	 DI_Print("FORWARD", DI_ROW_DIRECTION);
     	  DelayMs(1000);
         break;
       case DIR_DOWN:
     	  printf("Backward\r\n");
     	 level = BACKWARD;
     	 DI_Print("BACKWARD", DI_ROW_DIRECTION);
     	 DelayMs(1000);
         break;
       case DIR_LEFT:
     	  DelayMs(1000);
     	  printf("Left\r\n");
     	 level= LEFT;
     	 DI_Print("LEFT", DI_ROW_DIRECTION);
     	  break;
       case DIR_RIGHT:
     	  printf("right\r\n");
     	 level= RIGHT;
     	  DelayMs(1000);
     	 DI_Print("RIGHT", DI_ROW_DIRECTION);
         break;
...
static void button_init()
{
  ...
  GPIO_PinModeSet( APDS_INT_PORT,APDS_INT_PIN,gpioModeInputPull,1);
}
...
void set_device_name(bd_addr *pAddr)
{
  ...
  sprintf(name, "REMOTE %x:%x", pAddr->addr[1], pAddr->addr[0]);
    ...
    case gecko_evt_mesh_node_display_output_oob_id:
    		{
              ...
              printf("\r\n");
          	  ...
          	  printf(" OTP : %s",oob_str);
          	  DI_Print(oob_str, DI_ROW_STATUS);
    		}
             break;
...
//          break;
      ...
      case gecko_evt_mesh_generic_client_server_status_id:
      {
    	  ...
    	  struct gecko_msg_mesh_generic_client_server_status_evt_t *pdata = (struct gecko_msg_mesh_generic_client_server_status_evt_t *)&(evt->data);
    	  int length = pdata->parameters.len;
    	  for(int i=0; i <length; i++)
    	  {
    	  data[i]=pdata->parameters.data[i];
    		  printf("data is  %d\r\n",pdata->parameters.data[i]);
    	  }
    	  level =data[1]<<8 | data[0];
    	  printf("LEVEL IS \r\n %d",level);

    	  if(level == GESTURE_THRESHOLD_INCREASE)
    	  {
    		  Sensor_state.gest_thresold += 5;
    	  }
    	  if(level == GESTURE_THRESHOLD_DECREASE)
    	  {
    		  Sensor_state.gest_thresold -= 5;
    	  }
    	  if(level == GESTURE_SENSETIVITY1_INCREASE)
    	   {
    		  Sensor_state.gest_sensetivity_1 += 5;
    	   }
    	  if(level == GESTURE_SENSETIVITY1_DECREASE)
    	    {
    		  Sensor_state.gest_sensetivity_1 -= 5;
    	    }
    	  gesture_state_store();
      ...
      case gecko_evt_system_external_signal_id:
           ...
           if (evt->data.evt_system_external_signal.extsignals & EXT_SIGNAL_APDS_PIN )
           {
        	   printf("Entered Gesture External signal\r\n");
     	      handleGestureFlag = 0;
     	      handleGesture();
           }
           break;
   ...
   friend =( struct gecko_msg_mesh_lpn_friendship_established_evt_t  *)&(evt->data);
   friend_address= friend->friend_address;
     printf("Friend Address is  %d \r\n",friend->friend_address);
